...
async function loadDashboard() {
  ...
  try {
    const [summaryJson, distributionJson] = await Promise.all([
      requestJson(`${adminApiBase}/analytics/summary`),
      requestJson(`${adminApiBase}/analytics/distribution`),
    ]);
    ...
    const normalized = normalizeUiError(error, 'Gagal memuat dashboard analytics.');
...
async function runAiAnalysis() {
  ...
  try {
    const payload = await requestJson(`${adminApiBase}/ai/analyze`, {
      method: 'POST',
      headers: {
    ...
    const normalized = normalizeUiError(error, 'Analisa gagal.');
...
async function loadLatestAi() {
  ...
  try {
    const payload = await requestJson(`${adminApiBase}/ai/latest?mode=${encodeURIComponent(activeMode)}`);
    applyLatestAnalysisState(payload);
    if (payload?.analysis) {
      localStorage.setItem(getModeCacheKey(payload?.mode || activeMode), payload.analysis);
      setAiStatus(
    ...
    const normalized = normalizeUiError(error, `Belum ada analisa ${modeLabel} tersimpan.`);
...
bindRuntimeErrorHandlers((normalized, originalError) => {
  setStatus(normalized.message, true, originalError);
});
