...
const PORT = Number(process.env.VISUAL_DIFF_PORT || 8900);
const BASE_URL = `http://localhost:${PORT}`;
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'artifacts', 'visual-legacy-dashboard');
const DEFAULT_LEGACY_SLUG = 'sman6-kotakupang';
...
function getBaselineAdminDashboardScript() {
  try {
    return execFileSync('git', ['show', 'HEAD:public/admin/dashboard.js'], {
      cwd: PROJECT_ROOT,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  } catch (error) {
    fail(`Gagal ambil baseline script dari git HEAD: ${error?.message || error}`);
  }
}

async function resolveCandidateSlugs() {
  const candidateSet = new Set();
  const envLegacy = String(process.env.LEGACY_SCHOOL_SLUG || '').trim();
  if (envLegacy) candidateSet.add(envLegacy);
  candidateSet.add(DEFAULT_LEGACY_SLUG);

  const loadAndCollect = async (url) => {
    ...
    data.forEach((item) => {
      ...
      if (slug) candidateSet.add(slug);
    });
  };

  await loadAndCollect(`${BASE_URL}/forms/api/schools/public`);
  await loadAndCollect(`${BASE_URL}/forms/api/tenants/public`);
...
async function resolveLegacyDashboardSlug(page, candidates) {
  for (const slug of candidates) {
    const cleanSlug = String(slug || '').trim();
    if (!cleanSlug) continue;
    await page.goto(`${BASE_URL}/forms/${cleanSlug}/admin/dashboard/`, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    const hasLegacyMarker = await page.locator('#kpi-q12').count();
    if (hasLegacyMarker > 0) {
      return cleanSlug;
    }
  }

  fail(
    `Tidak menemukan tenant legacy dashboard. Coba set LEGACY_SCHOOL_SLUG. Kandidat: ${candidates
      .filter(Boolean)
      .join(', ')}`
  );
}
...
async function normalizeVisualNoise(page) {
  ...
  await page.evaluate(() => {
    const volatileIds = ['status', 'ai-status', 'error-debug', 'ai-output'];
...
async function captureDashboardScreenshot({
  ...
  if (baselineScript) {
    await context.route('**/forms-static/admin/dashboard.js*', async (route) => {
      await route.fulfill({
        status: 200,
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/forms/${legacySlug}/admin/dashboard/`, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    await page.locator('#kpi-q12').waitFor({ timeout: 60000 });
...
async function run() {
  ...
  const baselineScript = getBaselineAdminDashboardScript();
  const server = await startLocalServer();
  const browser = await launchBrowser();

  try {
    const candidates = await resolveCandidateSlugs();
    const resolveContext = await browser.newContext({
      viewport: { width: 1280, height: 1000 },
      deviceScaleFactor: 1,
    });
    const resolvePage = await resolveContext.newPage();
    await loginAsSuperadmin(resolvePage, superadminEmail, superadminPassword);
    const legacySlug = await resolveLegacyDashboardSlug(resolvePage, candidates);
    ...
    const maxAllowedRatio = Number(process.env.VISUAL_DIFF_MAX_RATIO || 0.03);
    const report = {
      generatedAt: new Date().toISOString(),
      baseUrl: BASE_URL,
      legacySlug,
      baselineSource: 'git:HEAD public/admin/dashboard.js',
      maxAllowedRatio,
    ...
    console.log(`[INFO] legacy slug: ${legacySlug}`);
    ...
    if (!report.passed) {
      fail(
        `Visual diff melebihi ambang batas. ratio=${comparison.diffRatio.toFixed(6)}, max=${maxAllowedRatio}. Cek ${reportPath}`
      );
    }

    console.log('[PASS] Visual diff legacy dashboard within threshold.');
  } finally {
