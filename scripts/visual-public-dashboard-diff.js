...
const PORT = Number(process.env.VISUAL_DIFF_PUBLIC_DASHBOARD_PORT || 8908);
const BASE_URL = `http://localhost:${PORT}`;
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'artifacts', 'visual-public-dashboard');
...
async function resolveTargetQuestionnaire() {
...
async function normalizeVisualNoise(page) {
  ...
  await page.evaluate(() => {
    ...
    const volatileIds = ['dashboard-status', 'dashboard-error-debug'];
...
async function captureDashboardScreenshot({
  ...
  const hasBaselineOverrides = Boolean(baselineHtml && baselineCss && baselineJs);
  if (hasBaselineOverrides) {
    await context.route('**/forms/*/*/dashboard/', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await context.route('**/forms-static/forms/public-dashboard.css*', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await context.route('**/forms-static/forms/public-dashboard.js*', async (route) => {
      await route.fulfill({
        status: 200,
...
async function run() {
  ...
  const baselineHtml = getBaselineAssetFromHead('public/forms/public-dashboard.html');
  const baselineCss = getBaselineAssetFromHead('public/forms/public-dashboard.css');
  const baselineJs = getBaselineAssetFromHead('public/forms/public-dashboard.js');
  const hasBaseline = Boolean(baselineHtml && baselineCss && baselineJs);

  const server = await startLocalServer();
  const browser = await launchBrowser();

  try {
    const target = await resolveTargetQuestionnaire();
    const dashboardUrl = `${BASE_URL}/forms/${target.tenantSlug}/${target.questionnaireSlug}/dashboard/`;
    ...
    await captureDashboardScreenshot({
      browser,
      dashboardUrl,
      baselineHtml: hasBaseline ? baselineHtml : '',
      baselineCss: hasBaseline ? baselineCss : '',
      baselineJs: hasBaseline ? baselineJs : '',
    ...
    const maxAllowedRatio = Number(process.env.VISUAL_DIFF_PUBLIC_DASHBOARD_MAX_RATIO || 0.05);
    const report = {
      generatedAt: new Date().toISOString(),
      baseUrl: BASE_URL,
      tenantSlug: target.tenantSlug,
      questionnaireSlug: target.questionnaireSlug,
      baselineSource: hasBaseline
        ? 'git:HEAD public/forms/public-dashboard.(html|css|js)'
        : 'fallback current-working-tree (new page without HEAD baseline)',
      maxAllowedRatio,
    ...
    if (!report.passed) {
      fail(
        `Visual diff public dashboard melebihi ambang batas. ratio=${comparison.diffRatio.toFixed(
          6
        )}, max=${maxAllowedRatio}. Cek ${reportPath}`
      );
    }

    console.log('[PASS] Visual diff public dashboard within threshold.');
  } finally {
